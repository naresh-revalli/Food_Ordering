...
// To add Ai feauture for  all food items
exports.generateAllFoodAI = catchAsync(async (req, res) => {
  ...
  for (const food of foods) {
    try {
      console.log(`Generating AI for ${food.name}...`);
      ...
      // Wait 2 seconds to avoid Groq rate limits
      await new Promise((resolve) => setTimeout(resolve, 2000));

    } catch (err) {
      skipped++;
      console.log(`Skipped ${food.name}: ${err.message}`);

      // If Groq rate limits, wait a bit longer
  ...
  res.status(200).json({
    ...
    updated,
    skipped,
    message: `${updated} food items updated successfully`,
  });
});